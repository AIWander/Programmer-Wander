---
name: programmer
description: Use AIWander Programmer-Wander for local Windows development work through its exact 49-tool file, command, session, WSL, HTTP, transformation, system-inspection, SQLite, safety, and planning surface. Trigger for repository inspection or editing, builds and tests, local command execution, long-running sessions, mechanical transformations, or choosing between Programmer-Wander and browser, credential, knowledge, Git, or orchestration tools.
---
...
| Repository history and changes | A Git client or dedicated Git add-on; Programmer can invoke Git through `cmd` |
| Linux execution from Windows | Programmer WSL abilities |
| HTTP calls and downloads | Programmer `http_request` ability |
| Bulk text or data changes | Programmer transformation abilities |
| Processes, ports, clipboard, registry, SQLite | Programmer system abilities |
...
1. Inspect the smallest useful evidence with file and search abilities, or invoke read-only Git commands through `cmd`.
...
4. Use `cmd` for Cargo and ordinary Windows commands, `powershell` for Windows-specific cmdlets, and persistent-session or WSL background abilities for long-running work.
